...
@pytest.fixture
def init_wfc():
    wfc = WaveformDataset(sampling_frequency=4096, duration=8)
    return wfc
...
def test_prior(init_wfc):
    ...
    for geocent_time, target_snr in itertools.product([(101, 102)], [None, (0, 30)]):
        (freq_block,
         freq_meta,
         noise_freq_block) = init_wfc.frequency_waveform_response_block(10, 100,
                                                                        geocent_time=geocent_time,
                                                                        target_optimal_snr_tuple=target_snr)
        (time_block,
         time_meta,
         noise_time_block) = init_wfc.time_waveform_response_block(10, 100,
                                                                   geocent_time=geocent_time,
                                                                   target_optimal_snr_tuple=target_snr)
        assert freq_block.shape == noise_freq_block.shape == (10, len(dets), 16385)
        assert time_block.shape == noise_time_block.shape == (10, len(dets), 16384*2)
        ...
        if isinstance(target_snr, tuple):
