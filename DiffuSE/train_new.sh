...
diffwave="/root/gwspe/Diffusion-based/DiffuSE" 
model_name="model_1"  #"voicebank_model_vocoder_traindev"
pretrain_model="model_1/weights-204875.pt"
fix="--fix_in"
#. ./path.sh
...
if [ ${stage} -le 2 ]; then
    ...
    target_wav_root="/root/gwspe"

    train_spec_list="/root/gwspe/test-code/"

    #spec_path=${spec_root}/train
    #train_spec_list="${train_spec_list} ${spec_path}"
    
    if [ -z "$pretrain_model" ]; then
        python src/diffwave/__main__.py model_3/  ${target_wav_root}  ${train_spec_list}  --${task} ${fix}  --voicebank
    else
        python src/diffwave/__main__.py model_4/ ${target_wav_root}  ${train_spec_list}  --${task} --pretrain ${pretrain_model} ${fix}  --voicebank
    fi
fi
